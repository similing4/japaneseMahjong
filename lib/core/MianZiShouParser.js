...
var _parserWork = require("./parser.work.js");

var _parserWork2 = _interopRequireDefault(_parserWork);
...
var _TableForSearch = require("../data/TableForSearch.js");

var _TableForSearch2 = _interopRequireDefault(_TableForSearch);
...
var MianZiShouParser = function () {
  ...
  */
	function MianZiShouParser(paiList) {
		_classCallCheck(this, MianZiShouParser);
  ...
  */
	...
	_createClass(MianZiShouParser, [{
		...
		value: function calcXiangting() {
			...
			var calcResult = (0, _parserWork2.default)(hand, rest);
			var ret = {};
			ret.xiangTingCount = calcResult.xiangTingCount; //向听数
			ret.paiState = calcResult.type; //当前手牌状态
			ret.divideResult = calcResult.data.map(function (item) {
				return _Pai2.default.fromRealAscii(item);
			});
			return ret;
		}
  ...
  */
		...
		value: function calcMianzi() {
			...
			if (!_TableForSearch2.default[key.key]) //如果不存在表键说明不是面子手和牌
				return false; //不是面子手和牌牌型直接返回false
			var res = this._parseTableVal(_TableForSearch2.default[key.key], key.paiArr); //计算牌型对应的数据含义
			return res;
		}
		/*
   ...
   */
		...
		key: "_calcKey",
		value: function _calcKey(wanzi, tongzi, suozi, zipai) {
			var paiArr = []; //被计入的牌顺序
			var chunkArr = []; //构造数据数组，明晰算法，此处为了可读性降低了少许效率
			var sePaiTypeArr = ["Wanzi", "Tongzi", "Suozi"];
			[wanzi, tongzi, suozi].map(function (sePai, sePaiIndex) {
				//普通色牌遍历
				if (sePai.reduce(function (a, b) {
					return a + b;
				}) > 0) {
					//有该种色牌的情况下
					sePai.map(function (paiCount, index) {
						if (paiCount == 0) {
							//当前张没有牌
						...
						if (index > 0 && sePai[index - 1] > 0) //上张有牌这张也有，说明连续了，计入0
							chunkArr.push("0");
						paiArr.push(new _Pai2.default(sePaiTypeArr[sePaiIndex], index + 1));
						chunkArr.push(Array(paiCount * 2 - 1).join("1")); //需要计入2n-2个1，1张即0个1
					});
					if (sePai[8] > 0) chunkArr.push("01"); //切换花色计入不连续标志01。如果有9那么没有记录最后一次的01故而需要补记
				}
			});
			if (zipai.reduce(function (a, b) {
				return a + b;
			}) > 0) {
				//有字牌的情况下
				zipai.map(function (paiCount, index) {
					if (paiCount == 0) return; //不需要考虑字牌的连续性
					if (index > 3) //三元牌
						paiArr.push(new _Pai2.default("Sanyuan", index - 3));else paiArr.push(new _Pai2.default("Feng", index + 1));
					chunkArr.push(Array(paiCount * 2 - 1).join("1")); //需要计入2n-2个1，1张即0个1
  ...
  */
		...
		key: "_parseTableVal",
		value: function _parseTableVal(val, paiArr) {
			return val.map(function (paixing) {
				//拿取每种情况
				...
				for (var i = 0; i < keziCount; i++) {
					...
					ret.mianzi.push(new _Mianzi2.default("Kezi", false, paiArr[kezi]));
				}
				for (var i = 0; i < shunziCount; i++) {
					var shunzi = paixing & 0xF;paixing >>= 4; //逐次读取4位作为刻子
					ret.mianzi.push(new _Mianzi2.default("Shunzi", false, paiArr[shunzi]));
				}
				return ret;
			});
		}
	...
	return MianZiShouParser;
}();

exports.default = MianZiShouParser;