 ...
 */
	...
	_createClass(YakuCalculator, [{
		...
		value: function getDora(hePaiPaixing, state) {
			...
			hePaiPaixing.getPaiList().map(function (pai) {
				...
				if (c > 0) {
					if (!doraNormal[pai.pai_real_ascii + '']) doraNormal[pai.pai_real_ascii + ''] = c;else doraNormal[pai.pai_real_ascii + ''] += c;
				}
				c = pai.getDoraCountLi(state);
				if (c > 0 && state.isLiZhi) {
					if (!doraLi[pai.pai_real_ascii + '']) doraLi[pai.pai_real_ascii + ''] = c;else doraLi[pai.pai_real_ascii + ''] += c;
				}
				if (pai.isRed) {
					if (!doraRed[pai.pai_real_ascii + '']) doraRed[pai.pai_real_ascii + ''] = 1;else doraRed[pai.pai_real_ascii + '']++;
				}
			});
			for (var i = 0; i < state.beidora; i++) {
				var pei = _Pai2.default.fromRealAscii(30);
				var c = pei.getDoraCountNormal(state);
				if (c > 0) {
					if (!doraNormal[pei.pai_real_ascii + '']) doraNormal[pei.pai_real_ascii + ''] = c;else doraNormal[pei.pai_real_ascii + ''] += c;
				}
				c = pei.getDoraCountLi(state);
				if (c > 0 && state.isLiZhi) {
					if (!doraLi[pei.pai_real_ascii + '']) doraLi[pei.pai_real_ascii + ''] = c;else doraLi[pei.pai_real_ascii + ''] += c;
				}
			}
			Object.keys(doraNormal).map(function (pai_real_ascii) {
				ret.push({
					count: doraNormal[pai_real_ascii],
					pai: _Pai2.default.fromRealAscii(parseInt(pai_real_ascii)),
					type: "Dora"
				});
			});
			Object.keys(doraLi).map(function (pai_real_ascii) {
				ret.push({
			...
			Object.keys(doraRed).map(function (pai_real_ascii) {
				ret.push({
			...
			if (state.beidora > 0) ret.push({
				count: state.beidora,
				pai: new _Pai2.default("Feng", 4),
				type: "BeiDora"
			});
			return ret;
		}
  ...
  */
		...
		key: "getDora3",
		value: function getDora3(hePaiPaixing, state) {
			var ret = [];
			var doraNormal = {};
			var doraLi = {};
			var doraRed = {};
			hePaiPaixing.getPaiList().map(function (pai) {
				var c = pai.getDoraCountNormal3(state);
				if (c > 0) {
					if (!doraNormal[pai.pai_real_ascii + '']) doraNormal[pai.pai_real_ascii + ''] = c;else doraNormal[pai.pai_real_ascii + ''] += c;
				}
				c = pai.getDoraCountLi3(state);
				if (c > 0 && state.isLiZhi) {
					if (!doraLi[pai.pai_real_ascii + '']) doraLi[pai.pai_real_ascii + ''] = c;else doraLi[pai.pai_real_ascii + ''] += c;
				}
				if (pai.isRed) {
			...
			for (var i = 0; i < state.beidora; i++) {
				var pei = _Pai2.default.fromRealAscii(30);
				var c = pei.getDoraCountNormal3(state);
				if (c > 0) {
					if (!doraNormal[pei.pai_real_ascii + '']) doraNormal[pei.pai_real_ascii + ''] = c;else doraNormal[pei.pai_real_ascii + ''] += c;
				}
				c = pei.getDoraCountLi3(state);
				if (c > 0 && state.isLiZhi) {
					if (!doraLi[pei.pai_real_ascii + '']) doraLi[pei.pai_real_ascii + ''] = c;else doraLi[pei.pai_real_ascii + ''] += c;
				}
			}
		...
		value: function calcYaku(state) {
			...
			for (var i in this.hePaiPaixingList) {
				...
				if (yaku.length > 0) fan = yaku.map(function (yaku) {
					if (yaku.isYiMan || yaku.isLiangBeiYiMan) isYakuman = true;
					return yaku.fan;
				}).reduce(function (c, d) {
					return c + d;
				});
				...
				if (finalFan != 0) {
					if (isYakuman) finalFan = fan;else finalFan = fan + doraFan;
				}
				var point = YakuCalculator.calcPoint(finalFan, fu.fu, state);
				if (state.isZimo) test.push({
					yaku: yaku,
					dora: dora,
			...
			return test.sort(function (a, b) {
				if (b.point == a.point) {
					if (b.fan == a.fan) return b.fu.fu_real - a.fu.fu_real;
					return b.fan - a.fan;
				}
  ...
  */
		...
		key: "calcYaku3",
		value: function calcYaku3(state) {
			var test = [];
			for (var i in this.hePaiPaixingList) {
				var yaku = (0, _calc2.default)(this.hePaiPaixingList[i], this.state);
				var dora = this.getDora3(this.hePaiPaixingList[i], state);
				...
				var point = YakuCalculator.calcPoint3(finalFan, fu.fu, state);
				if (state.isZimo) test.push({
					yaku: yaku,
					dora: dora,
					paixing: this.hePaiPaixingList[i],
  ...
  */
		...
		value: function calcPoint(fan, fu, state) {
			...
			} else return Math.ceil(a * (state.ziFeng == 1 ? 6 : 4) / 100) * 100;
		}
  ...
  */
		...
		key: "calcPoint3",
		value: function calcPoint3(fan, fu, state) {
			var a = 0;
			if (fan > 0) {
				if (fan < 5) {
					a = fu * Math.pow(2, fan + 2);
