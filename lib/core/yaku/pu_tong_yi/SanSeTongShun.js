...
var SanSeTongShun = function (_Yi) {
	...
	function SanSeTongShun(hePaiPaixing) {
		_classCallCheck(this, SanSeTongShun);

		return _possibleConstructorReturn(this, (SanSeTongShun.__proto__ || Object.getPrototypeOf(SanSeTongShun)).call(this, "SanSeTongShun", "三色同顺", hePaiPaixing.isMenQianQing() ? 2 : 1, []));
	}
	/*
 	判断和牌牌型是否有当前役
 	参数：
 ...
 */
	...
	_createClass(SanSeTongShun, null, [{
		...
		value: function getCurrentYi(hePaiPaixing, state) {
			...
			for (var i in val) {
				if (val[i].length == 3) return [new SanSeTongShun(hePaiPaixing)];
			}return [];
		}
	}]);
