...
export default (hePaiPaixing,state)=>{
	...
	var yiman = _calcYiMan(hePaiPaixing,state); //先算役满，役满满足了的话普通役种就不用算了，直接算两倍役满。否则算普通役种。
	if(yiman.length > 0){
		var liangBeiYiman = _calcLiangBeiYiMan(hePaiPaixing,state);
	}
};